...
export class TestConsole {
	...
	constructor(params = {}) {
		...
		if (TestConsole.instance) throw Error('Repeated TestConsole');
		...
		this.element.style.cssText = 'position: fixed; bottom: 0; left: 0; ' +
			...
			'background: #000A; color: white; font-family: Courier New;';
		...
		this.element.id = this.name + 'Messages';
		this.messages.style.cssText = 'overflow: hidden auto;' +
			'scrollbar-color: #0008 #0004; flex: 1; mix-blend-mode: exclusion;' +
			'font-family: Courier New; font-weight: bold; font-size: 2vh;';
	...
	* @param size The size of the panel. */
	...
	log(message = '', tabs = 0, color = null, maxTabs = 0, callback, stack) { 
		...
		element.style.cssText = 'padding: 0.5vmin 1vmin; tab-size: 4;'; 
		...
		if (typeof message  == 'object') callback(message);
			...
			if (!stack) {
				...
				if (params) oldLog(message, params); 
				else if (message) oldLog(message);
			} else oldError(stack);
		}
...
console.log = (message) => { 
	let test = TestConsole.instance; if (!test) return;
